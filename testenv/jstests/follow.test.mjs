...
import {
	...
	FOLLOW_CELL_DELTA, FOLLOW_MAX_FRACTION, FOLLOW_MARGIN,
...
test("nothing to follow when nothing changed", () => {
	assert.equal(shouldFollow(null, 2.5), false);
});
...
test("nothing to follow at 1x, where the whole picture is already on screen", () => {
	...
	assert.equal(shouldFollow(r, 1), false);
	assert.equal(shouldFollow(r, 2.5), true);
});
...
test("a repaint is not a cursor", () => {
	...
	assert.equal(shouldFollow(r, 2.5), false);
...
test("a region above the box pans the picture down", () => {
	...
	const move = followPan({"view": {"scale": 2, "x": 0, "y": -400}, "viewport": BOX, "region": region});
...
test("bringing the trailing edge in never pushes it back out", () => {
	...
	const move = followPan({"view": {"scale": 2, "x": 0, "y": -50}, "viewport": BOX, "region": region});
	const top = -50 + 0 * 600;
	...
	test("a change at the bottom of the picture pulls the view down to it", async () => {
		...
		await pg.eval(`(() => {
			...
			window.__paint = function(n) {
				...
				if (n % 2 === 0) {
					x.fillStyle = "#fff"; x.fillRect(24, 452, 12, 20);
				}
			};
			window.__paint(1);
			return true;
	...
	test("the switch turns it off", async () => {
		...
		await pg.eval(`(() => {
			...
			window.__paint = function(n) {
				const x = c.getContext("2d");
				x.fillStyle = "#000"; x.fillRect(0, 0, 640, 480);
				if (n % 2 === 0) { x.fillStyle = "#fff"; x.fillRect(24, 452, 12, 20); }
			};
			window.__paint(1);
